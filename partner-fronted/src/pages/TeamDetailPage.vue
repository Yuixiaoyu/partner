<template>
  <template v-if="team">
    <van-row justify="center">
      <van-col span="6" style="padding: 30px 0"><img :src="team.teamImage" alt="" style="width: 6rem; border-radius: 50%" /></van-col>
    </van-row>
    <van-cell-group inset class="my-cell-group">
      <van-cell title="房间名" :value="team.name" />
      <van-cell title="房间描述" :value="team.description" />
      <van-cell title="加入人数" :value="<string>team.hasJoinNum" />
      <van-cell title="最大人数" :value="team.maxNum" />
      <van-cell title="房间状态" :value="team.status === 0 ? '公开' : '加密'" />
      <van-cell title="创建人" :value="team.createUserVo.username" />
      <van-cell title="创建时间" class="last-cell" :value="moment(team.createTime).format('YYYY-MM-DD HH:mm:ss')" />
      <van-button v-if="!isDetail" color="linear-gradient(to right, #ff6034, #ee0a24)" @click="doJoinTeam" style="width: 90%; margin: 0 auto" block>
        加入聊天室
      </van-button>
      <van-button v-if="isDetail" color="linear-gradient(to right, #ff6034, #ee0a24)" @click="doExitTeam" style="width: 90%; margin: 0 auto" block>
        退出聊天室
      </van-button>
    </van-cell-group>
  </template>
